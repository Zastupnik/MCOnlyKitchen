...
public class GuiFishingMiniGame extends GuiScreen {
    ...
    private static final ResourceLocation CHEST_TEXTURE =
            new ResourceLocation(MCOnlyKitchen.MODID, "textures/gui/chest.png");
    private static final ResourceLocation GOLDEN_CHEST_TEXTURE =
            new ResourceLocation(MCOnlyKitchen.MODID, "textures/gui/golden_chest.png");
    ...
    private final boolean isGoldenChest;
    ...
    private boolean chestOpening;
    private int chestAnimationTimer;
    private static final int CHEST_ANIMATION_DURATION = 40;

    public GuiFishingMiniGame(int rodTier, boolean isLava, boolean hasChest, boolean isGoldenChest, int fishTier) {
    ...
    public void initGui() {
        ...
        mouseDown = false;
        chestOpening = false;
        chestAnimationTimer = 0;

        // Если сразу есть сундук и включена анимация, показываем анимацию
        if (hasChest && ModConfig.enableChestAnimation) {
            chestOpening = true;
            chestAnimationTimer = 0;
            finished = true;
        }
    ...
    protected void keyTyped(char typedChar, int keyCode) {
        if (finished || chestOpening) return;
    ...
    public void updateScreen() {
        if (chestOpening) {
            chestAnimationTimer++;
            if (chestAnimationTimer >= CHEST_ANIMATION_DURATION) {
                onSuccess(logic.hasChestReward(), logic.isGoldenChest());
            }
            return;
        }
        ...
        if (logic.isOverlap() && hitTimer == 0) hitTimer = 15;
        if (hitTimer > 0) hitTimer--;

        if (logic.isSuccess()) {
            if (ModConfig.enableChestAnimation && logic.hasChestReward()) {
                chestOpening = true;
                chestAnimationTimer = 0;
                finished = true;
            } else {
                showPerfect = !logic.hasChestReward();
                onSuccess(logic.hasChestReward(), logic.isGoldenChest());
            }
        }

        if (logic.isFailed()) onFail();
    }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        drawTexturedModalRect(guiX + CHANNEL_X - 2, fishY, FISH_U, getFishTextureV(fishTier), FISH_W, FISH_H);
        ...
        // HIT текст
        drawHitText(guiX, guiY);

        // PERFECT текст
        if (showPerfect) drawTexturedModalRect(guiX - 2, guiY - 20, PERFECT_U, 0, PERFECT_W, PERFECT_H);
    ...
    private void drawChest(int guiX, int guiY) {
        ...
        float alpha = logic.isBobberOnChest() ? 0.7f + (float)Math.sin(System.currentTimeMillis()/100.0)*0.3f : 1.0f;
        GL11.glColor4f(1,1,1,alpha);
        drawTexturedModalRect(guiX + CHANNEL_X + 1, chestY, CHEST_U, chestV, CHEST_W, CHEST_H);

        if (logic.getChestProgress() > 0) {
            GL11.glDisable(GL11.GL_TEXTURE_2D);
            int barW = (int)(logic.getChestProgress() * (CHEST_W - 2));
            GL11.glColor4f(1.0f, 0.84f, 0.0f,1);
            drawRect(guiX + CHANNEL_X + 2, chestY + CHEST_H + 1, guiX + CHANNEL_X + 2 + barW, chestY + CHEST_H + 3, 0xFFFFD700);
    ...
    private void drawHitText(int guiX, int guiY) {
        if (hitTimer <= 0) return;
        float scale = hitTimer > 10 ? 0.5f + (hitTimer - 10)*0.2f : hitTimer < 5 ? hitTimer*0.3f : 1.5f;
        GL11.glPushMatrix();
    ...
    private void drawChestAnimation(int guiX, int guiY) {
        ResourceLocation chestTex = logic.isGoldenChest() ? GOLDEN_CHEST_TEXTURE : CHEST_TEXTURE;
        mc.getTextureManager().bindTexture(chestTex);
        int frame = Math.min(chestAnimationTimer*10/CHEST_ANIMATION_DURATION, 9);
        int size = 128;
        GL11.glColor4f(1,1,1,1);
        GL11.glEnable(GL11.GL_BLEND);
        GL11.glBlendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA);
        drawTexturedModalRect(guiX+PANEL_W/2 - size/2, guiY+PANEL_H/2 - size/2, 0, frame*size, size, size);
        GL11.glDisable(GL11.GL_BLEND);
    }

    private int getProgressColor(int progress) {
        float ratio = progress/100.0f;
        if (ratio < 0.5f) return 0xFF000000 | (255<<16) | ((int)(ratio*2*255)<<8);
    ...
    private void onSuccess(boolean gotChest, boolean isGolden) {
        finished = true;
        NetworkHandler.INSTANCE.sendToServer(new PacketFishingResult(true, gotChest, isGolden, rodTier, isLava, fishTier));
        String msg = gotChest ? (isGolden?"gui.fishing.golden_chest":"gui.fishing.chest") : "gui.fishing.success";
    ...
    private void onFail() {
        finished = true;
        NetworkHandler.INSTANCE.sendToServer(new PacketFishingResult(false,false,false,rodTier,isLava,fishTier));
