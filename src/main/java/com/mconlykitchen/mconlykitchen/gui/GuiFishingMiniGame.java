...
public class GuiFishingMiniGame extends GuiScreen {
    ...
    private static final int CHEST_FRAMES = 10;
    private static final int CHEST_SIZE = 128;
    ...
    public GuiFishingMiniGame(int rodTier, boolean isLava, int fishTier, int entityId) {
        ...
        this.bobberEntityId = entityId; // сохраняем
    }
    ...
    public void initGui() {
        ...
        // ВНИМАНИЕ: никаких автозапусков анимации сундука
    }
    ...
    public void updateScreen() {
        ...
        if (chestOpening) {
            chestAnimationTimer++;
            if (chestAnimationTimer >= CHEST_ANIMATION_DURATION) {
                onSuccess(true, logic.isGoldenChest());
            }
        }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        // --- Поклёвка кнопки ПРОБЕЛ ---
        if (com.mconlykitchen.mconlykitchen.client.BiteAnimationHandler.isAnimating()) {
            Minecraft mc = Minecraft.getMinecraft();
            int widthCenter = width / 2;
            int heightCenter = height / 2 - 50; // чуть выше панели

            // простая анимация 4 кадра, меняем каждые 5 тиков
        ...
        if (chestOpening) {
            drawChestAnimation(guiX, guiY, partialTicks);
            GL11.glPopAttrib();
    ...
    // Рыба рисуется без дополнительных клампов — логика гарантирует границы
    private void drawFish(int guiX, int guiY) {
        int fishY = guiY + CHANNEL_Y + logic.getFishY();
        int v = getFishTextureV(fishTier);
        // дополнительная подстраховка от неверного v
        if (v < 0) v = 0;
    ...
    private void drawChestAnimation(int guiX, int guiY, float partialTicks) {
        ...
        int frame = Math.min((int)((float)chestAnimationTimer / CHEST_ANIMATION_DURATION * CHEST_FRAMES),
                CHEST_FRAMES - 1);
        ...
        int x = width / 2 - CHEST_SIZE / 2;
        int y = height / 2 - CHEST_SIZE / 2;
        drawTexturedModalRect(x, y, 0, frame * CHEST_SIZE, CHEST_SIZE, CHEST_SIZE);
    ...
    private void fireResult(boolean success, boolean gotChest, boolean isGolden) {
        ...
        NetworkHandler.INSTANCE.sendToServer(
                new PacketFishingResult(success, gotChest, isGolden)
        );
    }
