...
public class FishingMiniGameLogic {
    ...
    private final int rodTier;   // 0..4 (4 — универсальная)
    ...
    private static int normalizeRodTier(int tier) {
        if (tier <= 0) return 0;
        if (tier >= 4) return 4;
        return tier;
    }
    ...
    public FishingMiniGameLogic(int top, int bottom, int rodTier, boolean isLava, int fishTier) {
        this.top = top;
        this.bottom = bottom;
        this.rodTier = normalizeRodTier(rodTier);
        this.isLava = isLava;
        // Проверка измерения мира внутри, если нужно:
    ...
    private void initChest() {
        int chestChance = clampInt(ModConfig.getChestChance(rodTier, isNether), 0, 100);
        ...
        if (chestVisible) {
            ...
            int goldenChance = clampInt(ModConfig.getGoldenChestChance(rodTier, isNether), 0, 100);
    ...
    public void press() {
        float jump = 1.6f + 0.15f * clampInt(rodTier, 0, 4);
        barVelocity -= jump;
    }
    ...
    private void updateChest() {
        ...
        if (isBobberOnChest()) {
            ...
            if (chestProgress >= 100f) {
                ...
                // берём предмет из конфига
                this.rewardItem = ModConfig.getChestLoot(isGoldenChest);
    ...
    private net.minecraft.item.ItemStack rewardItem;

    public net.minecraft.item.ItemStack getRewardItem() {
        return rewardItem;
    }
    ...
    private void triggerChestAnimation() {
        if (net.minecraft.client.Minecraft.getMinecraft() != null) {
            net.minecraft.client.Minecraft.getMinecraft().displayGuiScreen(
                    new com.mconlykitchen.mconlykitchen.gui.GuiChestReward(isGoldenChest, rewardItem)
            );
        }
    }
    ...
    public boolean isFailed() {
        ...
        int rodBufferBonus = (rodTier == 4 ? 8 : rodTier == 3 ? 4 : 0);
